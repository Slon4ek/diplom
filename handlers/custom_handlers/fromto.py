...
def set_departure_point(message: Message) -> None:
    ...
    bot.send_message(user_id, 'Учтите что название города и название станции '
                              'могут быть одинаковыми, поэтому напишите что конкретно вас интересует.\n\n'
                              '<b><i>Например: "Город Москва" или "Станция Шереметьево"</i></b>\n\n'
                              '<b>Пожалуйста введите пункт отправления в соответствии с примером:</b>:',
                     parse_mode='HTML')
...
def get_departure_code(message: Message) -> None:
    ...
    if 'город' in message_split:
        ...
        bot.send_message(user_id, 'Не удалось получить код пункта отправления. '
                                  ...
                                  'Пример: <b><i>"Город Москва" или "Станция Шереметьево"</i></b>',
                         parse_mode='HTML')
...
def set_date(call: CallbackQuery) -> None:
    ...
    if not result:
        ...
        if schedule_between_stations['segments']:
            ...
            bot.edit_message_text(chat_id=call.message.chat.id,
                                  message_id=call.message.message_id,
                                  text='Не удалось найти транспорт для путешествия между выбранными '
                                       'пунктами.')
